...
async function loadWorkspace() {
    try {
        ...
        unsubscribeWorkspace = onSnapshot(doc(db, 'workspaces', currentWorkspace), (doc) => {
            if (doc.exists()) {
                ...
                document.getElementById('workspace-subtitle').textContent = `${workspace.members.length} member(s)`;
                document.body.setAttribute('data-theme', workspace.theme || 'default');
                
                // Update invite code
                document.getElementById('invite-code-text').textContent = workspace.inviteCode;
...
function setupTodoListener() {
    ...
    unsubscribeTodos = onSnapshot(todosQuery, (snapshot) => {
        ...
        if (document.getElementById('todo-view').classList.contains('active')) {
            renderTodos();
        }
    });
}
...
function openTodoModal(assignee = null) {
    if (assignee) {
        document.getElementById('todoAssignee').value = assignee === 'user1' ? currentUser.uid : 'shared';
    }
    openModal('todoModal');
}
...
document.getElementById('eventForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (fileInput.files[0]) {
            ...
            const fileRef = ref(storage, `events/${currentWorkspace}/${Date.now()}_${file.name}`);
            const snapshot = await uploadBytes(fileRef, file);
            eventData.attachmentUrl = await getDownloadURL(snapshot.ref);
            eventData.attachmentName = file.name;
        }
...
document.getElementById('todoForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (fileInput.files[0]) {
            const file = fileInput.files[0];
            const fileRef = ref(storage, `todos/${currentWorkspace}/${Date.now()}_${file.name}`);
            const snapshot = await uploadBytes(fileRef, file);
            todoData.attachmentUrl = await getDownloadURL(snapshot.ref);
            todoData.attachmentName = file.name;
        }
...
document.getElementById('join-form').addEventListener('submit', async (e) => {
    ...
    const inviteCode = document.getElementById('join-code').value.toUpperCase().trim();
    
    try {
        showButtonLoading(e.target.querySelector('button[type="submit"]'), true);
        
        // Find workspace with this invite code
        const workspaces = await getDocs(query(
...
document.getElementById('email-invite-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    showNotification(`Invite would be sent to ${email} with code: ${inviteCode}`, 'success');
});
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        const activeModal = document.querySelector('.modal.active:not(#auth-modal)');
        if (activeModal) {
...
function setupAutoSave() {
    const inputs = document.querySelectorAll('#eventForm input, #eventForm textarea, #todoForm input, #todoForm textarea');
