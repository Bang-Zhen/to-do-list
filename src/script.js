  ...
  window.showEventColorsModal = function () {
    ...
    setTimeout(() => {
      if (typeof showEventColorsModal !== "undefined") {
        ...
        showNotification(
          "Unable to load event colors. Please refresh the page.",
          "error"
        );
      }
    }, 100);
  ...
  window.showThemeModal = function () {
    ...
    setTimeout(() => {
      if (typeof showThemeModal !== "undefined") {
        ...
        showNotification(
          "Unable to load theme settings. Please refresh the page.",
          "error"
        );
      }
    }, 100);
  ...
  window.showMembersModal = function () {
    ...
    setTimeout(() => {
      if (typeof showMembersModal !== "undefined") {
        ...
        showNotification(
          "Unable to load members list. Please refresh the page.",
          "error"
        );
      }
    }, 100);
  ...
  window.showInviteModal = function () {
    ...
    setTimeout(() => {
      if (typeof showInviteModal !== "undefined") {
        ...
        showNotification(
          "Unable to load invite settings. Please refresh the page.",
          "error"
        );
      }
    }, 100);
  ...
  window.showJoinModal = function () {
    ...
    setTimeout(() => {
      if (typeof showJoinModal !== "undefined") {
        ...
        showNotification(
          "Unable to load join workspace. Please refresh the page.",
          "error"
        );
      }
    }, 100);
  ...
  window.openEventModal = function (
    ...
    setTimeout(() => {
      if (typeof openEventModal !== "undefined") {
        ...
        showNotification(
          "Unable to load event creator. Please refresh the page.",
          "error"
        );
      }
    }, 100);
      ...
      if (typeof deleteEvent !== "undefined") {
        ...
        if (window.showNotification) {
          window.showNotification(
            "Unable to delete event. Please refresh the page.",
            "error"
          );
        }
      }
  ...
  window.openEventDetails = function (eventId) {
    ...
    setTimeout(() => {
      if (typeof openEventDetails !== "undefined") {
        ...
        if (window.showNotification) {
          window.showNotification(
            "Unable to load event details. Please refresh the page.",
            "error"
          );
        }
      }
    ...
    window.setTheme = function (themeName) {
      ...
      setTimeout(() => {
        if (typeof setTheme !== "undefined") {
          ...
          showNotification(
            "Unable to switch theme. Please refresh the page.",
            "error"
          );
        }
      }, 100);
      ...
      async (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          document.getElementById(
            "workspace-subtitle"
          ).textContent = `${workspace.members.length} member(s)`;
...
function setupEventListener() {
  ...
  unsubscribeEvents = onSnapshot(
    ...
    (snapshot) => {
      ...
      console.log(
        "✅ Event listener processed. Events count:",
        newEvents.length
      );
  ...
  if (collisions.length > 0) {
    ...
    const maxCollisionBottom = Math.max(
      ...collisions.map(
        (c) =>
          (c.top || topOffset) +
          (categorizeEventDuration(c) !== "single-day" ? 20 : 18)
      )
    );
...
function renderEventWithPositioning(
  event,
  dayEvents,
  dayElement,
  ...
  if (eventUserType === "shared") {
    ...
    eventElement.style.background = `linear-gradient(135deg, ${baseColor
      .replace("hsl", "hsla")
      .replace(")", ", 0.9)")} 0%, ${baseColor
      .replace("hsl", "hsla")
      .replace(")", ", 0.7)")} 70%, ${baseColor
      .replace("hsl", "hsla")
      .replace(")", ", 0.6)")} 100%)`;
    eventElement.style.border = `1px solid ${borderColorHsla
      .replace("hsl", "hsla")
      .replace(")", ", 0.4)")}`;
    eventElement.style.boxShadow = `0 2px 8px ${borderColorHsla
      .replace("hsl", "hsla")
      .replace(")", ", 0.2)")}, inset 0 1px 0 rgba(255, 255, 255, 0.25)`;
  } else {
    // Personal events: Use personalized colors with enhanced contrast
    eventElement.style.background = `linear-gradient(135deg, ${baseColor} 0%, ${baseColor
      .replace("hsl", "hsla")
      .replace(")", ", 0.85)")} 100%)`;
    eventElement.style.border = `1px solid ${borderColorHsla
      .replace("hsl", "hsla")
      .replace(")", ", 0.6)")}`;
    eventElement.style.boxShadow = `0 1px 4px ${borderColorHsla
      .replace("hsl", "hsla")
      .replace(")", ", 0.25)")}, inset 0 1px 0 rgba(255, 255, 255, 0.15)`;
  }
...
function generateCalendar(date) {
  ...
  for (let day = 1; day <= 42 - totalDays; day++) {
    ...
    const nextMonthDateStr = `${nextMonthDate.getFullYear()}-${String(
        ...
        while (currentDate <= endDate) {
          ...
          if (dayElement) {
            ...
            if (weekIndex !== lastWeekIndex) {
              ...
              const eventUserType = event.shared
                ? "shared"
                : getEventTypeForUser(event);
              const eventColor = event.color || getEventColor(eventUserType);
              const borderColor =
                event.borderColor || getEventColor(eventUserType);
              ...
              if (eventUserType === "shared") {
                ...
                if (eventColor.startsWith("var(")) {
                  ...
                  const baseColor = hexToHsla(eventColor);
                  backgroundStyle = `linear-gradient(135deg, ${baseColor
                    .replace("hsl", "hsla")
                    .replace(")", ", 0.9)")} 0%, ${baseColor
                    .replace("hsl", "hsla")
                    .replace(")", ", 0.7)")} 70%, ${baseColor
                ...
                const baseColor = hexToHsla(eventColor);
                backgroundStyle = `linear-gradient(135deg, ${baseColor} 0%, ${baseColor
                  .replace("hsl", "hsla")
                  .replace(")", ", 0.85)")} 100%)`;
              }
          ...
          if (
            currentWeekSpan &&
            (dateStr === event.endDate || currentDate > endDate)
          ) {
            const weekEnd = lastWeekIndex * 7 + 6;
  ...
  setTimeout(() => {
    ...
    const singleDayEvents = sortedEvents.filter(
      (event) => !event.endDate || event.startDate === event.endDate
    );

    singleDayEvents.forEach((event) => {
      console.log(
        `🔍 Processing single-day event: "${event.title}" on ${event.startDate}`
      );
      ...
      if (dayElement) {
        const eventsContainer = dayElement.querySelector(".day-events");
        console.log(
          `📦 Events container found: ${eventsContainer ? "YES" : "NO"}`
        );
        ...
        if (eventsContainer) {
          ...
          if (existingEvent) {
            console.log(
              `⚠️ Event already exists on calendar, skipping: ${event.title}`
            );
            return;
          }
          ...
          console.log(
            `📊 Positioning event "${event.title}" at position ${currentCount} (top: ${topPosition}px)`
          );

          // Use enhanced rendering with predetermined positioning
          console.log(
            `🎨 Calling renderEventWithPositioning for: "${event.title}"`
          );
          const eventElement = renderEventWithPositioning(
            event,
            [],
            dayElement,
            topPosition
          ...
          console.log(
            `🎨 Event element created: ${eventElement ? "SUCCESS" : "FAILED"}`
          );
          ...
          console.log(
            `✅ Enhanced event element appended to calendar for: "${event.title}"`
          );
          ...
          console.log(
            `🏷️ Added has-events class to day element for: "${event.title}"`
          );

          // Verify the event element is actually in the DOM
          const verifyElement = eventsContainer.querySelector(
            `[data-event-id="${event.id}"]`
          );
          console.log(
            `🔍 Verification: Event element found in DOM: ${
              verifyElement ? "YES" : "NO"
            }`
          );
          if (verifyElement) {
            console.log(
              `🔍 Verification: Event element position:`,
              verifyElement.style.top,
              verifyElement.style.left
            );
          }
        } else {
          console.error(
            `❌ Events container not found in day element for date: ${event.startDate}`
          );
        }
      } else {
...
function changeMonth(direction) {
  ...
  const slideOutClass =
    direction > 0 ? "sliding-out-left" : "sliding-out-right";
...
async function renderFilteredTodos(tab) {
  ...
  switch (tab) {
    case "my-tasks":
      ...
      console.log(
        `🎯 My Tasks container (user1-todos) found:`,
        !!targetContainer
      );
      break;
    case "partner-tasks":
      ...
      console.log(
        `🎯 Partner Tasks container (user2-todos) found:`,
        !!targetContainer
      );
      break;
    case "shared-tasks":
      ...
      console.log(
        `🎯 Shared Tasks container (shared-todos) found:`,
        !!targetContainer
      );
      break;
    default:
...
async function getPartnerId() {
  ...
  try {
    ...
    if (workspaceDoc.exists()) {
      ...
      const partnerId = workspaceData.members.find(
  ...
  if (isNewEvent) {
    ...
    if (eventModal) {
      eventModal.setAttribute(
        "data-event-type",
        isShared ? "memory" : "personal"
      );
    }

    if (modalTitle) {
      modalTitle.textContent = isShared
        ? "✨ Create New Memory ✨"
...
${(() => {
  ...
  return `<span class="${badgeClass} event-type-toggle" data-event-id="${event.id}" onclick="event.stopPropagation(); toggleEventType('${event.id}')" style="cursor: pointer;">${badgeText}</span>`;
                                    ...
                                    <div class="event-item-details">
                                        ...
                                        ${
                                          event.location
                                            ? `<span class="detail-item">📍 ${event.location}</span>`
                                            : ""
                                        }
                                    </div>
                                    ${
                                ...
                                <div class="event-item-actions">
                                    ...
                                    <button class="action-btn delete" onclick="event.stopPropagation(); deleteEvent('${
                                      event.id
...
function getEventsForDate(selectedDate) {
  ...
  console.log(
    `🔍 Found ${matchingEvents.length} matching events for date ${selectedDate}`
  );
...
async function deleteEvent(eventId) {
  ...
  try {
    ...
    const eventItem = document
      .querySelector(`.event-item [data-event-id="${eventId}"]`)
      ?.closest(".event-item");
    if (eventItem) {
      ...
      console.log(
        "Event item not found in current modal, may have already been removed"
      );
    }
...
function openEventDetails(eventId) {
  ...
  if (event) {
    ...
    if (eventModal) {
      eventModal.setAttribute(
        "data-event-type",
        (event.type || (event.shared ? "together" : "bangzhen")) === "together"
          ? "memory"
          : "personal"
      );
    }
  ...
  try {
    ...
    if ((startTime && !endTime) || (!startTime && endTime)) {
      showCustomNotification(
        "Both start time and end time must be provided together",
        "error"
      );
      showButtonLoading(button, false);
      return;
    ...
    if (editingId) {
      ...
      setTimeout(() => {
        console.log(
          "🔄 Manual calendar regeneration triggered after event creation"
        );
        generateCalendar(currentDate);
      }, 1000);
    ...
    events = sortedEvents;
    console.log(
      "✅ Events sorted dynamically with",
      sortedEvents.length,
      "events"
    );
    ...
    console.log(
      "⏳ Waiting for database sync before regenerating calendar with enhanced sorting..."
    );
    setTimeout(() => {
      console.log(
        "🔄 Manual calendar regeneration triggered with dynamic sorting"
      );
...
document.getElementById("todoForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (editingId) {
      ...
      // Immediately update the DOM element with smooth transitions
      const existingElement = document.querySelector(
        `[data-todo-id="${editingId}"]`
      );
      if (existingElement) {
        ...
        // Update title
        const titleElement = existingElement.querySelector(".todo-text strong");
        if (titleElement) {
          titleElement.textContent = todoData.title;
        }

        // Update notes - find the notes meta div (first one after title)
        const metaDivs = existingElement.querySelectorAll(".todo-meta");
        if (metaDivs.length > 0) {
          // First meta div is notes, second is due date
          metaDivs[0].textContent = todoData.notes || "";
          // Update due date if provided
          if (todoData.dueDate) {
            if (metaDivs.length > 1) {
              metaDivs[1].textContent = `Due: ${new Date(
                todoData.dueDate
              ).toLocaleDateString()}`;
            } else {
              // If no due date meta div exists, create one
              const dueDateDiv = document.createElement("div");
              dueDateDiv.className = "todo-meta";
              dueDateDiv.textContent = `Due: ${new Date(
                todoData.dueDate
              ).toLocaleDateString()}`;
              const todoText = existingElement.querySelector(".todo-text");
              todoText.appendChild(dueDateDiv);
            }
          } else if (metaDivs.length > 1) {
...
function updateProgress(tab = null) {
  ...
  switch (tab) {
    ...
    case "partner-tasks":
      // Get partner ID
      const partnerId = todos.find(
        (todo) =>
          todo.assignee !== currentUser.uid && todo.assignee !== "shared"
      )?.assignee;
      filteredTodos = todos.filter((todo) => todo.assignee === partnerId);
...
function renderTodos(todos) {
  ...
  console.log(
    "renderTodos completed - individual tabs will be populated by renderFilteredTodos()"
  );
}
...
function createTodoElement(todo) {
  ...
  div.innerHTML = `
	      ...
	      <button class="delete-todo-btn" type="button" data-todo-id="${
          todo.id
  ...
  checkbox.addEventListener("click", (e) => {
    ...
    console.log(
      "✅ Event target parent classes:",
      e.target.parentElement?.classList
    );
    console.log(
      "✅ Checkbox computed style pointer-events:",
      getComputedStyle(e.target).pointerEvents
    );
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      inviteElement.innerHTML = `
                ...
                <span class="invite-date">${formatDate(
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (userAvatar) {
        const displayName =
          profileData.displayName || currentUser.displayName || "U";
  ...
  return new Promise((resolve) => {
    ...
    const checkInterval = setInterval(() => {
      ...
      if (colorsInitialized) {
        ...
        clearInterval(checkInterval);
        console.warn(
          "⚠️ Color initialization timeout, proceeding with defaults"
        );
        resolve(eventColors);
      }
    }, 100);
  ...
  overlay.innerHTML = `
		<div class="color-picker-modal">
			...
			<div class="color-picker-container">
				<input type="color" class="color-picker-input" value="${
          eventColors[userType] || defaultEventColors[userType]
        }">
			</div>
...
function applyEventColor(userType) {
  const colorInput = document.querySelector(
    ".color-picker-modal .color-picker-input"
  );
  if (colorInput) {
    ...
    showNotification(
      "🎨 Event color updated! Your calendar events will use the new color.",
      "success"
    );
  }
}
...
function saveEventColors() {
  ...
  console.log(
    "🎨 Found color inputs (personal events only):",
    colorInputs.length
  );
  ...
  try {
    ...
    // Show success notification
    showNotification(
      "✨ Event color preferences saved successfully!",
      "success"
    );
...
function cleanupSharedEventColors() {
  ...
  try {
    const currentColors = JSON.parse(
      localStorage.getItem("eventColors") || "{}"
    );
...
function updateAllEventColors() {
  ...
  if (
    document.getElementById("eventColorsModal") &&
    document.getElementById("eventColorsModal").classList.contains("active")
  ) {
...
(function enhanceGlobalFunctionExposure() {
  ...
  console.log(
    "🎉 All core functions successfully exposed globally with recovery mechanisms"
  );
})();
...
function testSharedColorCustomization() {
  ...
  showNotification(
    "🔍 Color test completed! Check console for details.",
    "info"
  );
}
...
async function toggleEventType(eventId) {
  ...
  try {
    ...
    const badgeElement = document.querySelector(
      `.shared-tag[data-event-id="${eventId}"], .personal-tag[data-event-id="${eventId}"]`
    );

    console.log(
      "🔍 Looking for badge element with selector:",
      `.shared-tag[data-event-id="${eventId}"], .personal-tag[data-event-id="${eventId}"]`
    );
